...
- **Migrations:** plain numbered `.sql` files in `TransportationSystemApi/TransportationSystemApi/Database/`, run manually with `sqlcmd`. No EF migrations, no auto-apply. Latest applied: `018_Consignments.sql`. A consolidated, idempotent `Database/PROD_ALL_MIGRATIONS.sql` (regenerate with `Database/_regen_prod_script.sh`) concatenates every migration in order for a one-shot production deploy.
